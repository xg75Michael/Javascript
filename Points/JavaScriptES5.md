...
- RegExp对象：<br>
	...
	位置的匹配：
		`^` 匹配开头，多行匹配每行的开头位置；`$`匹配结尾，多行每行的结尾位置；
			`'hello'.replace(/^|$/g, “#”); // “#hello#”`
		`\b` 是单词的边界，就是`\W`和`\w`, `\w`和`^`, `\w`和`$`之间的位置；
			`'some[String].mp4'.replace(/\b/g, “#”);` // "#some#[#String#].#mp4#"
		`\B` 是非单词的边界，和上一个相反；
			`'some[String].mp4'.replace(/\B/g, “#”);` // "s#o#m#e[S#t#r#i#n#g]#.m#p#4"
		`(?=p)` 是一个字模式，匹配`p`前面的位置；
			`"hello".replace(/(?=l)/g, '#');` // "he#l#lo"
		`(?!p)` 匹配所有非`p`前面的位置；
			`"hello".replace(/(?!l)/g, '#');` // "#h#e#l#l#o#"
		可以试着把位置理解为`””`空字符；
	位置匹配实例：
		`/.^/` 不匹配任何东西；
		`'123132321'.replace(/(?!^)(?=(\d{3})+$)/g, “,”);` // 123,132,321
			其中`(?!^)`表示不匹配开头位置；
