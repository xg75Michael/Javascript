...
- RegExp对象：<br>
	...
	RegExp对象的静态属性：<br>
		input / $_  最近一次要匹配的字符串<br>
		lastMatch / $&  最近一次的匹配项<br>
		lastParen / $+  最近一次匹配的捕获组<br>
		leftContext / $`  lastMatch之前的文本<br>
		rightContext / $’  lastMatch之后的文本<br>
		multiline / $*  返回布尔值，表示是否使用多行模式<br>
	操作符的优先级：<br>
		1.转义符`\`；<br>
		2.括号和方括号`(…), (?=...), (?!...), (?:...), […]`;<br>
		3.量词限定符`{n,m}, +, *, ?`;<br>
	...
	正则实例的方法：<br>
		`test`:<br>
			返回`true`或`false`;<br>
			没有`g`的时候每次从第0个字符匹配；<br>
			有`g`的时候会修改`lastIndex`属性，下次匹配从上个`lastIndex`下一个位置开始；<br>
		`exec`:<br>
			返回一个对象，有匹配到的属性，index，input或null;<br>
			匹配单次，通常需要配合`while`使用;<br>
	回溯的概念：<br>
	...
	以下方法可以和正则表达式使用：<br>
		...
		`someStr.replace(someRegex, otherStr)` 返回替换后的字符串；<br>
			`.replace`第二个如果是回调函数，参数是`(match,$1,$2,index,input)`;<br>
	具体的例子：<br>
		`/^\d+$/` // 只能是1个或者多个数字<br>
